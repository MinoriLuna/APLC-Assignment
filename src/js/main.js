...
const loadFromCSV = () => {
    const results = [];
    // UPDATED PATH: Uses ".." to go up one folder from 'js' to 'src'
    fs.createReadStream('../data/CAR_DETAILS_FROM_CAR_DEKHO.csv') 
        .pipe(csv())
        .on('data', (data) => {
            // UPDATED MAPPING: Maps CSV lowercase columns to your Code's uppercase keys
            results.push({
                Car_Name: data.name,
                Year: parseInt(data.year),
                Selling_Price: parseInt(data.selling_price),
                Fuel_Type: data.fuel,
                Transmission: data.transmission // Added just in case
            });
        })
        .on('end', () => {
            console.log("\n CSV Data loaded..");
